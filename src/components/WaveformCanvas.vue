...
const props = withDefaults(
  ...
  { loopA: null, loopB: null, loopActive: true, loops: () => [], ceilingDb: null, monitor: false, gainDb: 0, outTrail: null, reductionTrail: null, reduction: 0, headroomDb: PEAK_CEILING_DB }
)
...
/** where a saved loop's flag has been dragged to, until the drag is let go of */
...
watch(() => props.loops, () => (loopPreview = null), { deep: true })
...
function applyDrag(x: number) {
  ...
  if ((drag.kind === 'marker' || drag.kind === 'savedLoop') && !drag.armed) {
    ...
    cancelHold()
    drag = props.draggable ? { kind: 'pan', fromX: grabbed.fromX, fromPosition: props.position, moved: true } : { kind: 'seek' }
    return applyDrag(x)
  }
...
function onPointerDown(e: PointerEvent) {
  ...
  if (drag.kind === 'marker' || drag.kind === 'savedLoop') {
    ...
    const grabbed = drag
    holdTimer = setTimeout(() => ((grabbed.armed = true), (holdTimer = null), scheduleDraw()), HOLD_MS)
